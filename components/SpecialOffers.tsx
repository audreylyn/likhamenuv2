...
export const SpecialOffers: React.FC = () => {
  ...
  const handleImageChange = async (offer: SpecialOffer) => {
    ...
    if (newImageUrl !== null && newImageUrl !== offer.image_url) {
      try {
        const newOffers = offers.map(o => o.id === offer.id ? { ...o, image_url: newImageUrl } : o);
  ...
  if (!config) {
    if (isEditing) {
      // Show placeholders if editing
      return (
        <section id="specialOffers" className="py-24 bg-bakery-primary/5 relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <p>Special Offers section not configured. Please add content.</p>
            {/* Could add a button to initialize default config here */}
          </div>
        </section>
      )
    }
