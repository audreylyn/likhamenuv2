...
export const ChatSupport: React.FC = () => {
  ...
  const checkChatSupportEnabled = () => {
    ...
    if (chatConfig) {
      ...
      // Default to disabled if no config found in JSON
      // This matches the behavior of "if table row is missing, insert disabled row"
      setIsEnabled(false);
    }
  };
  ...
  return (
    <>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        onClick={toggleChat}
        className={`fixed bottom-8 right-8 z-40 p-5 rounded-full shadow-2xl transition-colors flex items-center justify-center ${isOpen ? 'bg-bakery-dark text-white rotate-90' : 'bg-bakery-primary text-white hover:bg-bakery-accent'
          }`}
      >
